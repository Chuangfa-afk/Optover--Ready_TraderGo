...
class AutoTrader(BaseAutoTrader):
    ...
    #     if instrument == Instrument.FUTURE:
    #         self.FUTURE_midpoint_price = (bid_prices[0] + ask_prices[0]) // 2
    #     if instrument == Instrument.ETF:
    #         # Calculate the midpoint price
    #         ETF_midpoint_price = (bid_prices[0] + ask_prices[0]) // 2
            ...
            # # Place new orders at the desired prices
            ...
            #     if self.position + LOT_SIZE <= POSITION_LIMIT-11:  # Check if new order would exceed POSITION_LIMIT
            #         self.send_insert_order(bid_id, Side.BUY, bid_price, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
            ...
            #     if self.position - LOT_SIZE >= -POSITION_LIMIT+11:  # Check if new order would exceed POSITION_LIMIT
            #         self.send_insert_order(ask_id, Side.SELL, ask_price, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
            #         self.asks.add(ask_id)
            #         self.ask_id = ask_id
            #         self.ask_price = ask_price

            # if self.FUTURE_midpoint_price > ETF_midpoint_price and self.FUTURE_midpoint_price != 0:
            #     print("case 1: Future > ETF")
            #     self.bid_id = next(self.order_ids)
            #     self.send_insert_order(self.bid_id, Side.BUY, ask_prices[0], LOT_SIZE, Lifespan.GOOD_FOR_DAY)
            #     self.position = self.position + 1
            # elif self.FUTURE_midpoint_price < ETF_midpoint_price and self.FUTURE_midpoint_price != 0:
            #     print("case 2: ETF > Future")
            #     self.ask_id = next(self.order_ids)
            #     self.send_insert_order(self.ask_id, Side.SELL, bid_prices[0], LOT_SIZE, Lifespan.GOOD_FOR_DAY)
            #     self.position = self.position + 1
            # else:
            #     if self.position > 0:
            #         self.ask_id = next(self.order_ids)
            #         self.send_insert_order(self.ask_id, Side.BUY, bid_prices[0], self.position, Lifespan.FILL_AND_KILL)
            #         self.position = 0

            #     elif self.position < 0:
            #         self.bid_id = next(self.order_ids)
            #         self.send_insert_order(self.bid_id, Side.SELL, ask_prices[0], -self.position, Lifespan.FILL_AND_KILL)
            #         self.position = 0
